...
namespace cn
{
  ...
  bool transfer_cmd::parse_token_tx(LoggerRef& logger, const std::vector<std::string> &args)
  {
    ...
    try 
    {
      ...
      while (!ar.eof()) 
      {
        ...
        else
        {
          ...
          TokenTransfer destination;
          TokenTransfer feeDestination;          
          ...
          if (aliasUrl.empty())
          {
            ...
            destination.token_details = token_details;
            t_dsts.push_back(destination);
          }
          else
          {
            t_aliases[aliasUrl].emplace_back(TokenTransfer{static_cast<int64_t>(de.amount), "", token_details});
          }
