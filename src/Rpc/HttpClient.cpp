...
HttpClient::HttpClient(platform_system::Dispatcher& dispatcher, const std::string& address, uint16_t port, bool ssl_enable) :
...
void HttpClient::request(HttpRequest &req, HttpResponse &res) {
  ...
  if (this->m_ssl_enable) {
    try {
      platform_system::SocketStreambuf streambuf((char *) "", 1);
...
void HttpClient::connect() {
  ...
  if (this->m_ssl_enable) {
    ...
    try {
      auto ipAddr = platform_system::Ipv4Resolver(m_dispatcher).resolve(hostname);
